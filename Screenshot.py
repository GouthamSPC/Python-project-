...
import pyautogui
import logging
from screeninfo import get_monitors  # Import get_monitors from screeninfo module
...
class ScreenshotApp(QMainWindow):
    ...
    def start_screenshot(self):
        ...
        if self.update_checkbox.isChecked():
            ...
            while os.path.exists(
                    os.path.join(folder_name, f'{self.test_case_name}_v{self.version_number}.docx')):
                self.version_number += 1
            self.document_name = os.path.join(folder_name, f'{self.test_case_name}_v{self.version_number}.docx')
    ...
    def takescreenshot(self, monitor):
        logging.info(f"Taking ScreenShot for monitor {monitor.name}")

        screenshot_name = os.path.join(os.getcwd(), "Output",
                                       f'{self.test_case_name}_screenshot_v{self.version_number}_{self.screenshot_counter}_{monitor.name}.png')
        if self.hide_taskbar.isChecked():
            taskbar_height = 40
            screenshot_area = (monitor.x, monitor.y, monitor.width, monitor.height - taskbar_height)
